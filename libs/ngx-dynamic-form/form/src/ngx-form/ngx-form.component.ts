...
export class NgxFormComponent implements OnInit, DoCheck {
  ...
  get formGroup(): FormGroup {
    return this.formModel ?
      this.formModel.formGroup : null;
  }

  get subModels(): INgxForm[] {
    return this.formModel ?
      this.formModel.subModels : null;
  }
  ...
  ngDoCheck() {
    ...
    if (changes && formArray) {
      changes.forEachAddedItem((change) => this.onSubModelAdded(formArray, change.item.formGroup));
      changes.forEachRemovedItem((change) => this.onSubModelRemoved(formArray, change.item.formGroup));
    }
  }

  initSubModels() {
    const formArray = this.formBuilder.array(this.subModels.map((item) => item.formGroup));
